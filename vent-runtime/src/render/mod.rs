...
use wgpu::{Adapter, CommandEncoder, Device, include_wgsl, Queue, SurfaceConfiguration, SurfaceError, SurfaceTexture, TextureView};
...
pub struct RuntimeRenderer {
    ...
    multi_renderer: dyn MultiDimensionRenderer,
}
...
impl RuntimeRenderer {
    pub(crate) fn new(dimension: Dimension, window: &Window) -> &Self {
        ...
        let multi_renderer = match dimension {
            Dimension::D2 => Renderer2D::init(&default_renderer.config, &default_renderer.adapter, &default_renderer.device, &default_renderer.queue),
            Dimension::D3 => Renderer3D::init(&default_renderer.config, &default_renderer.adapter, &default_renderer.device, &default_renderer.queue),
        };
        &Self {
            default_renderer,
            multi_renderer,
    ...
    pub fn render(&self, window: &Window) -> Result<(), SurfaceError> {
        ...
        Self::render_from(window, &mut encoder, &view, &self.multi_renderer).expect("Failed to Render Runtime");
    ...
    pub fn render_from(
        ...
        multi_renderer.render(encoder, &view);
        Ok(())
    }
