...
impl CameraController3D {
    ...
    pub fn update_camera(&mut self, camera: &mut Camera3D, dt: Duration) {
        ...
        let (yaw_sin, yaw_cos) = camera.rotation.x.sin_cos();
        let forward = Vec3::new(yaw_cos, 0.0, yaw_sin).normalize();
        let right = Vec3::new(-yaw_sin, 0.0, yaw_cos).normalize();
        camera.position += forward * (self.amount_forward - self.amount_backward) * self.speed * dt;
        camera.position += right * (self.amount_right - self.amount_left) * self.speed * dt;

        let (pitch_sin, pitch_cos) = camera.rotation.y.sin_cos();
        ...
        camera.rotation.x += self.rotate_horizontal.to_radians() * self.sensitivity * dt;
        camera.rotation.y += -self.rotate_vertical.to_radians() * self.sensitivity * dt;

        self.rotate_horizontal = 0.0;
        self.rotate_vertical = 0.0;

        if camera.rotation.y < -90.0 {
            camera.rotation.y = -90.0;
        } else if camera.rotation.y > 90.0 {
            camera.rotation.y = 90.0;
        }
    }
}
